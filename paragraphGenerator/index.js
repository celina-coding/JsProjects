...
const loremFrom=document.querySelector('.lorem-from');
const loremText=document.querySelector('.lorem-text');
const count =document.querySelector('#count');

loremFrom.addEventListener('submit',function(e){
    // preventing the default 
    e.preventDefault();
    //get a random number
    const random=Math.floor(Math.random()*text.length);
    // get the value that the user has submitted
    let value = parseInt(count.value);
    //if the value isNAN <=0 or >9 then we return 1 random paragraph
